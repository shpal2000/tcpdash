...
void tls_client_app::start ()
{
    auto cs_grp_list = get_app_json()["cs_grp_list"];
    ...
    for (auto it = cs_grp_list.begin(); it != cs_grp_list.end(); ++it)
    {
        ...
        cs_grp_stats_arr->push_back (get_app_stats());
        cs_grp_stats_arr->push_back (get_zone_app_stats());
        cs_grp_stats_arr->push_back (get_zone_sock_stats());
